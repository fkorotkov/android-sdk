...
public class User<T extends User> extends GenericJson {
    ...
    public User(AbstractClient client, Class<T> myClazz, KinveyAuthRequest.Builder builder) {
        this.client = client;
        this.builder = builder;
        this.myClazz = myClazz;
        this.builder.setUser(this);
    ...
    public boolean isUserLoggedIn() {
        return (this != null && (getId() != null || getAuthToken() != null));
    }
    ...
    public T initUser(KinveyAuthResponse response, String userType, T userObject) throws IOException {
        ...
        put("_kmd", response.getMetadata());
        putAll(response.getUnknownKeys());
        setId(response.getUserId());
        put("_kmd", response.getMetadata());
        putAll(response.getUnknownKeys());
    ...
    public User retrieveMetadataBlocking() throws IOException {
        ...
        putAll(ret.getUnknownKeys());
        setUsername(ret.getUsername());
        return this;
    }
    ...
    public Delete deleteBlocking(boolean hardDelete) throws IOException {
        Preconditions.checkNotNull(getId(), "currentUser ID must not be null");
    ...
    public Retrieve<T> retrieveBlocking() throws IOException {
        Preconditions.checkNotNull(getId(), "currentUser ID must not be null");
        Retrieve<T> retrieve = new Retrieve<T>(this, getId(), myClazz);
    ...
    public Update updateBlocking() throws IOException {
        Preconditions.checkNotNull(getId(), "currentUser ID must not be null");
        Update<T> update = new Update<T>(this, myClazz);
     ...
     * @throws IOException
     */
    public Update updateBlocking(User<T> user) throws IOException {
        Preconditions.checkNotNull(getId(), "currentUser ID must not be null");
        Update update = new Update<T>(user, myClazz);
        client.initializeRequest(update);
        return update;
    }
